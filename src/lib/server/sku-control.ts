...
export function parseSku(value?: string | null): ParsedSku | null {
  ...
  return { sku: `${match[1].toUpperCase()}-${match[2].padStart(4, '0')}`, prefix: match[1].toUpperCase(), sequence };
}
...
export function categoryFromSku(value?: string | null) {
  ...
  if (prefix === 'BSC' || prefix === 'BBC') return 'baseball_cards';
  if (prefix === 'ELC' || prefix === 'ELE') return 'electronics';
  if (prefix === 'MOV') return 'movies';
...
export async function allocateSkuRange(db: D1Database, prefixValue: string, quantity: number) {
  ...
  if (Number(verified?.lastNumber ?? 0) !== last) {
    ...
    return Array.from(
      { length: quantity },
      (_, index) => `${prefix}-${String(retryCurrent + 1 + index).padStart(4, '0')}`
    );
  }
