...
export const POST: RequestHandler = async ({ platform, request }) => {
  ...
  if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    return json({ error: 'Choose a valid expense date.' }, { status: 400 });
  }
  if (!description) {
    return json({ error: 'Enter an expense description.' }, { status: 400 });
  }
  if (!category || !EXPENSE_CATEGORIES.has(category)) {
    return json({ error: 'Choose a valid expense category.' }, { status: 400 });
  }
  if (!Number.isInteger(amountCents) || amountCents <= 0) {
    return json({ error: 'Enter an expense amount greater than $0.' }, { status: 400 });
  }

  const id = crypto.randomUUID();
  const financeId = `finance:manual:${id}`;
  const externalId = `manual:${id}`;
  // Noon UTC keeps a user-entered calendar date from shifting a day in US time zones.
  ...
  await platform.env.DB.prepare(`
    INSERT INTO financial_transactions (
      ...
      booking_entry, category, source, description, expense_category, memo,
      updated_at
    )
