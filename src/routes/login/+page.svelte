<script lang="ts">
  import { Check, KeyRound, LoaderCircle, LockKeyhole, Mail, UserRound } from '@lucide/svelte';
  ...
  <main class="auth-card">
    ...
    <form onsubmit={submit}>
      {#if createMode}
        <label>
          <span>Name</span>
          <div class="auth-input"><UserRound size={17} /><input bind:value={name} autocomplete="name" placeholder="Dave" /></div>
        </label>
      {/if}

      <label>
        <span>Email</span>
        <div class="auth-input"><Mail size={17} /><input bind:value={email} type="email" autocomplete="email" placeholder="you@example.com" /></div>
      </label>

      <label>
        <span>Password</span>
        <div class="auth-input"><KeyRound size={17} /><input bind:value={password} type="password" autocomplete={createMode ? 'new-password' : 'current-password'} placeholder="12+ characters" /></div>
      </label>
    ...
    {#if data.signupMode === 'open' && data.canSignUp}
      <button class="mode-switch" onclick={() => { createMode = !createMode; message = null; }} type="button">
...
<style>
  ...
  .auth-kicker { color: #b8f34a; font: 800 .67rem "SFMono-Regular", Consolas, monospace; letter-spacing: .14em; }
